...
from jio.brain.proto.knowledge.healthcare.req_res.get_symptom_name_pb2 import GetSymptomNameResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
import copy

class GetSymptomNameWorker:
    ...
    def transform_request(self, request):
        ...
        if not request.symptom_id:
            ...
            request_object = {"symptom_id": request.symptom_id}
    ...
    def do(self, request, status):
        ...
        response = dispatcher.get_entity_name_by_id(request, "symptom", "healthcare_entity_type")
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = GetSymptomNameResponse(status = status)
        else:
            response = MessageToDict(response, preserving_proto_field_name=True)
            response = GetSymptomNameResponse(symptom_name = response['entity_name'], status = status)
