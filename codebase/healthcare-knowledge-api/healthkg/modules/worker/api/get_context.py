...
from jio.brain.proto.knowledge.healthcare.req_res.get_context_pb2 import GetContextResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
import copy

class GetContextWorker:
    ...
    def do(self, request, status):
        ...
        response = dispatcher.get_context(request)
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = GetContextResponse(status = status)
        else:   
            response = MessageToDict(response, preserving_proto_field_name=True)
            response = GetContextResponse(context = response['context'], status = status)
