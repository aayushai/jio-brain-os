...
from jio.brain.proto.knowledge.healthcare.req_res.get_attribute_order_pb2 import GetAttributeOrderResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
import copy

class GetAttributeOrderWorker:
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = GetAttributeOrderResponse(status = status)
        else:  
            response = MessageToDict(response, preserving_proto_field_name=True)
            response = GetAttributeOrderResponse(attribute_order = response['attribute']["order"], status = status)
