...
from jio.brain.proto.knowledge.healthcare.req_res.get_disease_patient_state_pb2 import GetDiseasePatientStateResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
import copy

class GetDiseasePatientStateWorker:
    ...
    def transform_request(self, request):
        ...
        if not request.disease_id:
            ...
            request_object={"entity_id": request.disease_id}
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = GetDiseasePatientStateResponse(status = status)
        else:
            gender = []
            minimum_age = []
            maximum_age = []
            ...
            if not gender or not maximum_age or not minimum_age:
                ...
                response = GetDiseasePatientStateResponse(status = transform_response_status)
            else:
                response = GetDiseasePatientStateResponse(gender = gender,
