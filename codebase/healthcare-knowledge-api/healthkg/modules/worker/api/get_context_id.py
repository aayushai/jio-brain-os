...
from jio.brain.proto.knowledge.healthcare.req_res.get_context_id_pb2 import GetContextIdResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
import copy

class GetContextIdWorker:
    ...
    def transform_request(self, request):
        ...
        if not request.context_name:
            ...
            request_object={"context_name": request.context_name}
    ...
    def do(self, request, status):
        ...
        response = dispatcher.get_entity_id_by_name(request, "context", "healthcare_context_type")
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = GetContextIdResponse(status = status)
        else:
            response = MessageToDict(response, preserving_proto_field_name=True)
            response = GetContextIdResponse(context_id = response['entity_id'], status = status)
