...
from jio.brain.proto.knowledge.healthcare.req_res.get_symptom_id_pb2 import GetSymptomIdResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
import copy

class GetSymptomIdWorker:
    ...
    def do(self, request, status):
        ...
        response = dispatcher.get_entity_id_by_name(request, "symptom", "healthcare_entity_type")
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = GetSymptomIdResponse(status = status)
        else:
            response = MessageToDict(response, preserving_proto_field_name=True)
            response = GetSymptomIdResponse(symptom_id = response['entity_id'], status = status)
