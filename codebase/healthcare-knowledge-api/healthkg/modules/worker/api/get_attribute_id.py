...
from jio.brain.proto.knowledge.healthcare.req_res.get_attribute_id_pb2 import GetAttributeIdResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
import copy

class GetAttributeIdWorker:
    ...
    def do(self, request, status):
        ...
        response = dispatcher.get_attribute_id(request)
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = GetAttributeIdResponse(status = status)
        else:
            response = MessageToDict(response, preserving_proto_field_name=True)
            response = GetAttributeIdResponse(attribute_id = response['attribute_id'], status = status)
