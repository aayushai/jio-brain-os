...
from jio.brain.proto.knowledge.healthcare.req_res.get_disease_given_symptom_pb2 import GetDiseaseGivenSymptomResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
from healthkg.modules.worker.utils.query import *
import copy

class GetDiseaseGivenSymptomWorker:
    ...
    def transform_request(self, request):
        ...
        if not request.symptom_id:
            ...
            queries = []
            for i in request.symptom_id:
                queries.append(query_get_disease_given_symptom%(i))
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = GetDiseaseGivenSymptomResponse(status = status)
        else:
            response_obj = []
            for i in response:
                response_obj.append(set(ast.literal_eval(i.cursor)))
            response_obj = set.intersection(*response_obj)
            response = GetDiseaseGivenSymptomResponse(disease_id = response_obj, status = status)
