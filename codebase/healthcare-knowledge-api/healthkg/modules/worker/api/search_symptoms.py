...
from jio.brain.proto.knowledge.healthcare.req_res.search_symptoms_pb2 import SearchSymptomsResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
import copy

class SearchSymptomsWorker:
    ...
    def transform_request(self, request):
        ...
        if not request.keyword:
            ...
            request_object = {"entity_type": "symptom",
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = SearchSymptomsResponse(status = status)
        else:
            response_obj = get_common_response_objs(response, "search_response")
            response = SearchSymptomsResponse(search_response = response_obj, status = status)
