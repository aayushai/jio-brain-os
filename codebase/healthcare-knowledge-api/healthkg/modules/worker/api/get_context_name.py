...
from jio.brain.proto.knowledge.healthcare.req_res.get_context_name_pb2 import GetContextNameResponse
from healthkg.modules.dispatcher.kg_dispatcher import KGDispatcher
import copy

class GetContextNameWorker:
    ...
    def do(self, request, status):
        ...
        response = dispatcher.get_entity_name_by_id(request, "context", "healthcare_context_type")
    ...
    def transform_response(self, response, status):
        ...
        if not response:
            response = GetContextNameResponse(status = status)
        else:
            response = MessageToDict(response, preserving_proto_field_name=True)
            response = GetContextNameResponse(context_name = response['entity_name'], status = status)
