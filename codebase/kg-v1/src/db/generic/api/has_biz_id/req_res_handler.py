...
from db.generic.api.has_biz_id.config import *
from jio.brain.proto.knowledge.api.data.has_biz_id_pb2 import HasBizIdResponse
...
def validate(request):
	...
	if request.entity_id and request.biz_id:
...
def post_process_function(cursor,status): 
    ...
    if cursor["static_cursor"] is None:
        response = HasBizIdResponse(is_present = False , status =status)
    else:
        response = HasBizIdResponse(is_present = True, status= status)
