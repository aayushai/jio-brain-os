...
from db.arango.api.get_all_children.query import query
...
def pre_process_function(request):
	...
	print(from_node)
	_query = query%(predicate_name, from_node)
...
def post_process_function(cursor, brain_status):
	if(cursor == []):
		...
		brain_status["brain_status_instance"][0]["parameters"]["msg"] = "Entity type does not exist in database"
	print(brain_status)
