...
query = """FOR doc in healthcare_entity_type
FILTER doc.id == %s
LET attributes = (
    FOR x in has_attribute
    FILTER split(x._from, "/")[1] == doc.entity_type
    LET attribute_doc = UNSET(DOCUMENT(x._to), "_key", "_rev", "type")
    ...
    LET depends_on = (
        ...
        RETURN {"symptom_id" : symptom_doc_id, "attribute_id": attribute_doc_id, "attribute_value_id": attribute_value_doc_id}
...
RETURN MERGE(UNSET(doc, "_id", "_key", "_rev", "type"), 
            {"attributes": attributes, 
            "name": doc.name.english.canonical, 
            "display_name": doc.name.english.display_name,
