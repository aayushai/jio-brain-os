...
from .config import *
from jio.brain.proto.knowledge.api.data.add_canonical_name_pb2 import AddCanonicalNameRequest
from jio.brain.proto.knowledge.api.data.add_canonical_name_pb2_grpc import AddCanonicalNameServiceStub
...
logger = get_logger("root", "add_canonical_name")
channel = grpc.insecure_channel(add_canonical_name_channel)
stub = AddCanonicalNameServiceStub(channel)
def test(request):
	response = stub.serve(request)
	return response

def test_add_canonical_name(entity_id, language, canonical, alias):
    logger.debug("entered test method")
    try:        
        element = { "%s"%alias: True }

        request = AddCanonicalNameRequest(
            entity_id  = entity_id,
            language = Language(
                language = language,
            ),
...
if __name__ == "__main__":
    api_response = test_add_canonical_name(entity_id, language, canonical, alias)
    print(api_response)