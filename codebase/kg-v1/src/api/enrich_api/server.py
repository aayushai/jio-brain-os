from jio.brain.proto.knowledge.api.data.enrich_api_pb2_grpc import(
	EnrichServiceServicer, # Base class for generated servicer 
	add_EnrichServiceServicer_to_server # generated rpc handler	
)

from servings.grpc.service import serve
from db.generic.dao.query_handler import init, execute
from api.enrich_api.config import *
...
class Servicer(EnrichServiceServicer):
...
if __name__ == "__main__":
	'''
	server method starts the enrich_api server
	'''
	init(api_name=API_NAME)
	serve(
			rpc_handler = add_EnrichServiceServicer_to_server,
			servicer = Servicer,
			port = SERVER_PORT,
