from jio.brain.proto.knowledge.api.data.get_attribute_id_pb2_grpc import(
	GetAttributeIdServiceServicer, # Base class for generated servicer 
	add_GetAttributeIdServiceServicer_to_server # generated rpc handler	
)

from servings.grpc.service import serve
from db.generic.dao.query_handler import init, execute
from api.get_attribute_id.config import *
...
i/p: request
o/p: GetAttributeIdAPI response
'''

class Servicer(GetAttributeIdServiceServicer):
...
if __name__ == "__main__":
	...
	serve(
			rpc_handler = add_GetAttributeIdServiceServicer_to_server,
			servicer = Servicer,
			port = SERVER_PORT,
